...
sys.path.append('/home/tangni/dev/proml')
...
if cuda:
    ...
    D_A = D_A.cuda(device)
    D_B = D_B.cuda(device)
    criterion_GAN.cuda(device)
    criterion_cycle.cuda(device)
    criterion_identity.cuda(device)
...
dataloader = DataLoader(
    ImageDataset("{}/{}".format(cfg_net['data_path'], opt.dataset_name), transforms_=transforms_, unaligned=True),
...
val_dataloader = DataLoader(
    ImageDataset("{}/{}".format(cfg_net['data_path'], opt.dataset_name), transforms_=transforms_, unaligned=True, mode="test"),
    batch_size=5,
    shuffle=True,
    num_workers=1,
)
