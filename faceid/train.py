...
# ============================ step 1/5 数据 ============================
csv_file = '/disk1/home/xiaj/res/face/training/VGGFace2_disk2-GCWebFace_mainland_HongkongTaiwan_JapanKorea.csv'
...
# image_list, label_list, val_image_list, val_label_list = tools.load_data_from_csv(csv_file, end_idx=None, all4train=False)
...
train_transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.RandomErasing(p=0.9, scale=(0.02, 0.13), ratio=(0.3, 3.3), value=0),
...
# train_data = dataset.FaceidDatasetFormat(format_data=(image_list, label_list), transform=train_transform)

# db_path = '/disk2/res/face/VGGFace2-mtcnn-align182x182margin44.lmdb'
db_path = '/disk2/res/face/training/VGGFace2-mtcnn-align182x182margin44.lmdb'
train_data = ImageFolderLMDB(db_path, transform=train_transform, target_transform=None)

# valid_data = dataset.FaceidDatasetFormat(format_data=(val_image_list, val_label_list), transform=valid_transform)
...
train_loader = DataLoaderX(dataset=train_data, batch_size=BATCH_SIZE, shuffle=True, num_workers=10, pin_memory=True)
    ...
    if (epoch+1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if acc_val > 0.85:
                ...
                save_folder = os.path.join(MODEL_SAVE_PATH, SAVE_FIELD,
                                           'acc{:.4f}-loss{:.4f}-epoch{}.pth'.format(acc_val, loss_val_mean, epoch + 1))
