...
def load_dataset(mode, name, dataset_params):
    ...
    if name == 'mnist':
        # root = './data_mnist'
        root = cfg_net['data_path']
        train_set = dsets.MNIST(root=root, train=True, download=False, transform=train_transform)
        val_set = dsets.MNIST(root=root, train=False, download=False, transform=val_transform)
        train_loader = torch.utils.data.DataLoader(train_set, batch_size=dataset_params.batch_size, shuffle=True,
                                                   num_workers=dataset_params.num_workers,
                                                   pin_memory=dataset_params.cuda)
        val_loader = torch.utils.data.DataLoader(val_set, batch_size=dataset_params.batch_size, shuffle=True,
                                                 num_workers=dataset_params.num_workers, pin_memory=dataset_params.cuda)
    else:
        root = './data_cifar10'
        if dataset_params.aug == "on":
            train_transform = transforms.Compose([
...
def load_subsampled_dataset(mode, name, dataset_params):
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=dataset_params["batch_size"], shuffle=True,
                                               ...
                                               num_workers=dataset_params["num_workers"],
                                               pin_memory=dataset_params["cuda"])
    val_loader = torch.utils.data.DataLoader(val_set, batch_size=dataset_params["batch_size"], shuffle=False,
                                             num_workers=dataset_params["num_workers"],
                                             pin_memory=dataset_params["cuda"])
